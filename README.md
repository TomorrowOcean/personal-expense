...
- **記帳**：數字鍵盤快速輸入（Android 按鍵有震動回饋），支出／收入切換，分類與付款方式一點即選
- **日期與時間**：預設跟著時鐘走，補記過去的帳可以改日期與時間；編輯舊記錄時保留它原本的時區
- **收據辨識**：拍照或選圖，由 Gemini 視覺模型讀出金額、店名、日期時間、分類，確認後才送出
...
- **修正拍照後「記憶體不足」而讓分頁被系統回收**：原本把相機原圖整張丟進 `<img>` 解碼再畫到 canvas，
  一張 4000×3000 展開成點陣是 48MB，高畫素機種可達數百 MB。改用 `createImageBitmap` 的
  `resizeWidth` 在**解碼階段**就縮圖，記憶體只跟輸出尺寸成正比；小檔（<900KB）則直接解碼，
  避免把小圖放大反而變肥。canvas 用完立刻歸零釋放，並保留不支援時的 `<img>` 備援路徑
- **修正載入動畫出現在錯誤的按鈕上**：`doScan()` 寫死把動畫塞進第一顆按鈕，
  用「相簿選圖」時動畫卻跑到「拍收據」上。改為由呼叫端指定是哪一顆
- **收據辨識新增付款方式**：先前 schema 根本沒有這個欄位，所以不管憑證上寫什麼，
  付款方式都停在預設值。現在會從 VISA／卡號／授權碼／SALE 判斷為刷卡，
  CASH／找零為現金，LINE Pay 等為行動支付……辨識不出時回「未知」，
  前端保留使用者原本的選擇，不用猜測值覆蓋
- 提示詞補上「信用卡簽帳單也算消費憑證」與「藥妝店歸醫療健康」
